...
ComplexNumber ComplexNumber::operator/(const ComplexNumber &c){
	double z;
	z=pow(c.real,2)+pow(c.imag,2);
	return ComplexNumber((real*c.real+imag*c.imag)/z,(imag*c.real-real*c.imag)/z);
}
...
double ComplexNumber::angle(){
	double angle;
	if (real < 0 && imag <0) angle=(atan(imag/real) * 180 / PI)-180;
	else if (real < 0 && imag >0) angle=180+(atan(imag/real) * 180 / PI);
	else angle=atan(imag/real) * 180 / PI;
	return angle;
}
